...
namespace Bozo.ModularCharacters
{
    public class CharacterConversionTool : EditorWindow
    {
        ...
        /// <summary>
        /// Quick conversion method that can be called from other scripts
        /// </summary>
        [MenuItem("Tools/Character System/Quick Convert (Default Paths)")]
        public static void QuickConvert()
        {
            string defaultSourceFolder = "Assets/Resources/Characters";
            string defaultTargetDirectory = Path.Combine(Application.persistentDataPath, "SavedCharacters");

            // Ensure target directory exists
            if (!Directory.Exists(defaultTargetDirectory))
            {
                Directory.CreateDirectory(defaultTargetDirectory);
            }

            // Find and convert characters
            string[] guids = AssetDatabase.FindAssets("t:BSMC_CharacterObject", new[] { defaultSourceFolder });
            ...
            foreach (string guid in guids)
            {
                ...
                try
                {
                    ...
                    if (characterObject != null)
                    {
                        string fileName = Path.GetFileNameWithoutExtension(assetPath);
                        string jsonData = JsonUtility.ToJson(characterObject, true);
                        string targetFilePath = Path.Combine(defaultTargetDirectory, fileName + ".json");
            ...
            if (errors == 0)
            {
                Debug.Log($"Quick conversion completed! Converted {converted} character(s) to {defaultTargetDirectory}");
            }
            else
            {
                Debug.LogWarning($"Quick conversion completed with {errors} error(s). {converted} character(s) were successfully converted.");
            }
        }
    }
}