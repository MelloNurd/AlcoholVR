...
namespace EasyAudioCutter
{
    public class EasyAudioCutter : EditorWindow
    {
        ...
        private void ProcessAndSaveTrimmedClip()
        {
            ...
            // CHANGED: Pass sourceClip to SaveWav so we know where to start looking
            SaveWav(trimmedData, freq, channels, "Trimmed_" + sourceClip.name + (reverseAudio ? "_Reverse" : ""), sourceClip);
        }
        ...
        private void ProcessAndSaveMergedClip()
        {
            ...
            // CHANGED: Pass the first clip in the list as the reference for the save location
            SaveWav(mergedSamples.ToArray(), freq, channels, "MergedAudio", mergeClips[0]);
        }
        ...
        private void ProcessAndSaveAdjustedClips()
        {
            if (volumeClip != null)
            {
                ...
                // CHANGED: Pass volumeClip as the reference for the save location
                SaveWav(data, volumeClip.frequency, volumeClip.channels, volumeClip.name + "_Adjusted", volumeClip);
                EditorUtility.DisplayDialog("Done", "Adjusted clip saved!", "OK");
            }
        }

        // CHANGED: Added 'referenceClip' parameter to determine default save path
        private void SaveWav(float[] data, int frequency, int channels, string defaultName, AudioClip referenceClip = null)
        {
            int samples = data.Length / channels;
            AudioClip clip = AudioClip.Create(defaultName, samples, channels, frequency, false);
            clip.SetData(data, 0);

            // CHANGED: Determine the directory of the reference clip
            string defaultPath = "Assets";
            if (referenceClip != null)
            {
                string assetPath = AssetDatabase.GetAssetPath(referenceClip);
            ...
            // CHANGED: Passed 'defaultPath' as the last argument
            string path = EditorUtility.SaveFilePanelInProject("Save Audio", defaultName + ".wav", "wav", "Choose location", defaultPath);
